...
threadCount = 10
accidentCount = len(accidentList)

def getAccidentAndSaveToDatabase(start, end, accidentList):
    sql = "BEGIN\n"
    for i in range(start, end):
        accident = accidentList[i]
        ...
        if (checkRowIsExists('major_occupational_accidents', primaryKeys, values)):  # 確認是否有此筆資料，有此筆
            sql += updateOneRow('major_occupational_accidents',
                    columns, datas, primaryKeys, values) + ";\n"  # 更新資料
            print('更新第{index}筆'.format(index=i))
        else:  # 沒有此筆資料
            ...
            print('寫入第{index}筆'.format(index=i))
    sql += "END;"
    execSql(sql)

singleThreadProcessCount = accidentCount / threadCount

for count in range(1, threadCount + 1):
    start = int((count - 1) * singleThreadProcessCount)
    end = int(count * singleThreadProcessCount)
    # print(start, end)
    thread = threading.Thread(target=getAccidentAndSaveToDatabase, args=(start, end ,accidentList))
    thread.start()
...
endTime = time.time()
print('執行成功，總共耗時 %f 秒' % (endTime - startTime))
